...
- Goals:
  ...
  - To be able to list and retrieve previously analyzed SBOMs from the database, so they can re-run analysis or review past reports.

  - To have clear instructions and a simple command to set up the required database locally using Docker, so they can quickly get started with the persistence feature.
...
- FR-3.6.1: Database Choice: The agent MUST integrate with MongoDB as its primary data store for SBOMs.
- FR-3.6.2: Local Deployment: The project MUST provide clear instructions and configuration for deploying a local MongoDB instance using Docker.
...
- FR-3.6.3: Connection Management: The Python application MUST establish a connection to the MongoDB instance using connection details provided via environment variables (.env file).
- FR-3.6.4: SBOM Ingestion: After successfully parsing an SBOM (FR-3.2.2), the agent MUST store the raw SBOM content (JSON or XML) as a document in a designated MongoDB collection.
...
- FR-3.6.5: SBOM Retrieval (Future): While not fully implemented in v2.0, the architecture MUST support future capabilities to:
...
```json
{
  ...
  "filename": "my_application_sbom.json",
  "timestamp": ISODate("2025-07-19T10:30:00Z"),
  "sbom_format": "json",
  "components_count": 123,
  "raw_content": {
    // The full parsed CycloneDX SBOM content as a dictionary/JSON object
    "bomFormat": "CycloneDX",
    "specVersion": "1.4",
...
- NFR-6.1.3: Database Credentials: MongoDB connection details (host, port, username, password if applicable) MUST be loaded securely from environment variables.
- NFR-6.1.4: Data Isolation: The Docker setup for MongoDB should ensure that the database data is isolated and persistent across container restarts.
...
- NFR-6.2.3: Ingestion Speed: Storing an SBOM in the database should not significantly impact the overall analysis time (target: < 500ms per SBOM ingestion).
...
- NFR-6.3.3: Docker Setup Simplicity: The instructions for setting up MongoDB via Docker MUST be clear, concise, and require minimal manual steps.
...
- FR-7.1.1: The project repository MUST adhere to a standard and intuitive Python project structure to facilitate development, packaging, and maintenance. No major changes to the src directory structure, but new files will be added for database integration.
...
- FR-7.2.1: The README.md file is the primary entry point for new users and contributors and MUST contain the following sections:
  ...
  - Database Setup (v2.0): Detailing the Docker commands for MongoDB.
...
# MongoDB Connection Details for SBOM Persistence (v2.0)
MONGO_DB_HOST="localhost"
MONGO_DB_PORT="27017"
MONGO_DB_NAME="cti_agent_db"
# MONGO_DB_USERNAME="your_username" # Uncomment and set if using authentication
# MONGO_DB_PASSWORD="your_password" # Uncomment and set if using authentication
```
...
- FR-7.4.2: All project dependencies MUST be declared in the [project.dependencies] section of the pyproject.toml file, in accordance with modern Python packaging standards. The pyproject.toml file MUST be updated to include `pymongo` as a dependency.
...
### 8.2 Out of Scope for Version 2.0

To ensure a focused and achievable release, the following features and capabilities are explicitly out of scope for version 2.0:
...
- Integrated Search for Defensive Measures: The find_defensive_measures tool will rely on the LLM's intrinsic knowledge for v2.0. Future versions may integrate a tool that actively searches public repositories or threat intelligence feeds.

- Advanced Historical Analysis Features: While SBOM persistence is introduced, advanced features like trend analysis, diffing SBOMs over time, or complex historical querying are out of scope for this version.
